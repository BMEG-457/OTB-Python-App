...
class DataReceiverThread(QtCore.QThread):
    ...
    def __init__(self, device, client_socket, tracks):
        ...
        self.csv_filename = f"emg_recording_{timestamp}.csv"
        self.csv_file = open(self.csv_filename, "w", newline="")
        self.csv_writer = csv.writer(self.csv_file)

        # Write header row: t, ch1, ch2, ..., ch72
        header = ["timestamp"] + [f"ch_{i+1}" for i in range(self.device.nchannels)]
        self.csv_writer.writerow(header)
        print(f"Recording EMG to {self.csv_filename}")
        # ----------------------------
    ...
    def run(self):
        while self.running:
            try:
                ...
                # Feed the EMG data into visual tracks
                channel_index = 0
                ...
                # ---- CSV WRITE ----
                now = time.time()
                # reshaped_data shape: (channels, samples_per_packet)
                samples_per_packet = reshaped_data.shape[1]

                for s in range(samples_per_packet):
                    row = [now] + reshaped_data[:, s].tolist()
        ...
        self.csv_file.close()
        print(f"EMG recording saved to {self.csv_filename}")
